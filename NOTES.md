...
- **Audit-2 Phase E** (small parity + cosmetics, items 17–19): bulk "＋ Add songs" checkmark modal (showSelectSongs) in the open-playlist header — multi-select over the library w/ search, already-in-playlist pre-marked (create flow kept as create-then-add per plan, low stakes); playlist card durations on EVERY card (playlistDuration helper: cache now keyed by track count so add/remove invalidates; computePlaylistDuration re-renders on completion); rail-addpl tooltip fixed ("…to a playlist"); fresh-install fx default now BYPASSED (base.bypass=true, twin of iOS effectsBypass; existing installs keep stored value). All DOM/cosmetic. Gate: tsc+bundle green; logic 13/13+18/18. Smoke: "Audit-2 Phase E". ALL AUDIT-2 PHASES A–E CODE-COMPLETE.
- **Audit-2 Phase D** (rail + transport polish, items 12–16): ✂ crop button in rail-actions (enabled only for local+yt on a synced session) → existing editCrop; double-click Now Playing title → rename current track; fx per-slider double-click reset + "Reset all" link (skips volume, re-dispatches slider input so fx state/per-track memory/sync push all fire); hold ⏪ = repeat −0.5s scrub (200ms), hold ⏩ = temporary 2× (el.playbackRate override + republish), owner-only via bindHoldButton (window mouseup, 400ms threshold), followers get click ±10; >60s-pause auto-restart in LocalPlayer (lastPausedAt/AUTO_RESTART_MS, twin of iOS resume(); deliberate seek clears the timer). All DOM/transport (no new pure logic). Gate: tsc+bundle green; logic 13/13+18/18. Smoke: "Audit-2 Phase D".
- **Audit-2 Phase C** (downloads parity, items 10–11): bare playlist/album links now route to a per-track batch pipeline in ui.ts (downloadPlaylistBatch). Spotify sets SUPPORTED (reverses P5 rejection): resolvePlaylist scrapes the public Spotify embed __NEXT_DATA__ (parseSpotifyEmbed) → ytsearch1:<artist> - <title> per track, twin of iOS's embedded-python scraper. YouTube sets enumerate via yt-dlp --flat-playlist --print id|title (parseFlatPlaylist), skip library dups by videoID, download one at a time; each failure → its own failed-panel row; post-download prompt suppressed (batch never calls it). download.ts spawn extracted to runYtDlp; parseDlChunk simplified (no more --yes-playlist item counting). New pure parsers in urls.ts (parsePlaylistURL) + download.ts. Gate: tsc+bundle green; logic 18/18 (phaseC) + 13/13 (phaseA). Smoke: "Audit-2 Phase C". Spotify re-run dedupe is imperfect (name-based; yt-dlp filename differs) — known edge, desktop keys library by YT id.
- **Audit-2 Phase B** (queue panel parity, items 6–9): Previously Played section (engine.previousTracks getter + playFromHistory, owner-local, most recent at bottom, click replays); queue-row context menu (reuse trackMenu on resolvable rows); drag-to-queue (library rows draggable w/ custom LIB_DRAG_TYPE, #upnext-panel drop target incl. empty state — distinct from queue reorder's text/plain); status strip "PLAYING FROM QUEUE · N songs" (prev+current+queue). All DOM (no new pure logic). Gate: tsc+bundle green; logic 13/13. Smoke: smoke-test.md "Audit-2 Phase B".
- **Audit-2 Phase A** (queue & playlist semantics, items 1–5): playlist Play/Shuffle now queue-based w/ 2s "Play now?/Shuffle now?" double-tap (engine.queueMany/injectAtFront twin queuePlaylist/injectAtFrontOfQueue); playlistLoop/wrap/playFromPlaylist/Up-Next-split RETIRED; loop auto-disables at the 4 live sites; idle queue-add starts playback (pure sessionIdle guard); playFromQueue pushes prev-history; clicking the playing row (library/playlist/queue) toggles pause/resume. Gate: tsc+bundles green; logic 13/13 PASS (committed test). Smoke: smoke-test.md "Audit-2 Phase A".

## OPEN  (unanswered / deferred / known issues)
- Audit-2 phases A–E ALL code-complete on `arpi/audit-2` (A1=(a), C10 done). Awaiting user GUI smoke pass (smoke-test.md "Audit-2 Phase A–E") + merge decision.
...
- Local artwork for folder-imported files (no yt id) — deferred at P5 (needs a tag parser / npm dep; user call).
- Spotify playlist/album downloads — DONE in audit-2 Phase C (embed scrape → per-track ytsearch). Re-run dedupe for Spotify sets is best-effort (name-based).
